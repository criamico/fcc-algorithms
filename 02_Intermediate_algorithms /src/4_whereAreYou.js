...
function whereAreYou(collection, source) {
    var keyIter,
        results = [],
        sourceKeysArr = Object.keys(source),
        hasPrevKeys = true;

    for (var i =0; i< collection.length; i ++){
        for (var j =0; j< sourceKeysArr.length; j ++){
             keyIter = sourceKeysArr[j];
             hasPrevKeys = (collection[i].hasOwnProperty(keyIter) && (collection[i][keyIter] === source[keyIter]) && hasPrevKeys);
        }
        if (hasPrevKeys)
